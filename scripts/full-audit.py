...
def run_audit():
    ...
    with sync_playwright() as p:
        ...
        for name, path in PAGES:
            ...
            page.on(
                "pageerror", lambda err: console_errors.append(f"[PAGE_ERROR] {err}")
            )
            ...
            # Desktop screenshot
            page.screenshot(
                path=f"D:/PROJECTS/Meteoric/scripts/screenshots/{name.lower().replace(' ', '-')}-desktop.png",
                full_page=True,
            )
            ...
            page.screenshot(
                path=f"D:/PROJECTS/Meteoric/scripts/screenshots/{name.lower().replace(' ', '-')}-mobile.png",
                full_page=True,
            )
            ...
            for e in console_errors[:5]:
                print(f"    {e}")
            print(
                f"  Images: {len(images)} total, {len(images_without_alt)} missing alt"
            )
        ...
        hamburger = page.query_selector(
            "[aria-label*='menu'], [aria-label*='Menu'], button:has(svg)"
        )
        if hamburger:
            hamburger.click()
            page.wait_for_timeout(500)
            page.screenshot(
                path="D:/PROJECTS/Meteoric/scripts/screenshots/mobile-nav-open.png"
            )
        ...
        for r in metrics["largestResources"]:
            print(
                f"    [{r['type']}] {r['name']}: {r['size_kb']}KB ({r['duration_ms']}ms)"
            )
